...
from model import GCNLSTMClassifier
...
def evaluate(model, samples, device, batch_size=16):
    ...
    with torch.no_grad():
        for graph_sequences, labels in make_batches(samples, batch_size=batch_size, shuffle=False):
            ...
            logits = model(graph_sequences).to(device)
...
def main():
    ...
    torch.save(model.state_dict(), "saved_models/gcn_lstm_sanity_check.pt")
    print("\nSaved model to saved_models/gcn_lstm_sanity_check.pt")
